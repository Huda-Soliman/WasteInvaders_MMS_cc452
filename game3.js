  ...
  function moveBg(){
        ...
        muteBtn.mousedown = muteBtn.touchstart = function(data)
       {
        ...
        function addScore()
        {
            ...
            scoreText.position.x =width-300;
        ...
        {
            requestAnimFrame( updateScore );
            scoreText.setText(score, "bold 150px ms comic sans", "white");
            if(score>=100){
                levelText.setText("LEVEL 2 ", "bold 60px ms comic sans", "white");
            }
            // render the stage 
            renderer.render(stage);
      ...
      /*  
        function Gifts()
        {
             ...
             if(!pause && life<=5){
                ...
                extraLife.position.x=(Math.random()* width);
                extraLife.got=false;
              // badguy.scale.x = badguy.scale.y =0.75;
                extraLife.anchor.x = extraLife.anchor.y=0.5;
                container.addChild(extraLife);
                gifts.push(extraLife);
                requestAnimFrame( moveGift );
            ...
            if(!pause )
                {
                        ...
                        extraLife.position.y += 1;
                        if(extraLife.position.y == window.height ){
                           
                              extraLife.visible=false ;
                               container.removeChild(extraLife);
                           
                        }
                      for(var t=0;t<gifts.l)
                         else  if(!gifts[i].got)
                             {
                             if((extraLife.position.y > myplane.position.y-100)&&(extraLife.position.y < myplane.position.y+100))
                            {
                                if((extraLife.position.x > myplane.position.x-100)&&(extraLife.position.x < myplane.position.x+100))
                            {
                                   
                                     var live = new PIXI.Sprite(lifeText);
                                        //lives=lives.reverse();
                                        live.position.x=scoreText.position.x-10;
                                        live.position.y=200+(150*(lives.length-1));
                                        live.anchor.x =  live.anchor.y =0.25;
                                        live.scale.x=live.scale.y=0.15;
                                        lives.push(live);
                                        //lives=lives.reverse();
